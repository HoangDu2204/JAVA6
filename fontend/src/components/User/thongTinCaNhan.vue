      ...
      <div class="account-container">
        <div class="account-sidebar">
          ...
          <ul>
            ...
            <li class="nav-item">
              <RouterLink class="nav-link" to="/soDiaChi">Sổ địa chỉ (0)</RouterLink>
            </li>
          </ul>
        </div> -->
...
<script setup>
import { ref, onMounted } from 'vue'
import api from '@/axios' //  axios custom interceptor tự gắn token từ localStorage
import Swal from 'sweetalert2'
import { useRouter } from 'vue-router'
...
onMounted(async () => {
  try {
    const res = await api.get('/api/users/profile') //  Không cần withCredentials
    user.value = res.data
  } catch (error) {
    console.error('Lỗi khi lấy thông tin người dùng:', error)
    Swal.fire({
